...
const regionOrder = ['USA', 'Canada', 'Mexico', 'Brazil', 'Peru', 'Patagonia', 'Colombia', 'Ecuador', 'Caribbean', 'Central America']
  ...
  return (
    <div
      ...
      <div style={{ height: isOpen ? 320 : 180, position: 'relative', overflow: 'hidden', transition: 'height 0.4s ease' }}>
        <picture>
          <source srcSet={`/images/americas/${dest.slug}-mobile.webp`} media="(max-width: 480px)" type="image/webp" />
          <source srcSet={`/images/americas/${dest.slug}-tablet.webp`} media="(max-width: 1024px)" type="image/webp" />
          <source srcSet={`/images/americas/${dest.slug}-desktop.webp`} type="image/webp" />
          <img
...
export default function AmericasPage() {
  ...
  const uniqueRegions = [...new Set(americasDestinations.map(d => d.region))]
  ...
  const groupedByRegion = orderedRegions.map(region => ({
    region,
    items: americasDestinations.filter(d => d.region === region),
