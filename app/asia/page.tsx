...
const countryOrder = ['Japan', 'China', 'South Korea', 'India', 'Thailand', 'Vietnam', 'Indonesia', 'Malaysia', 'Singapore', 'Philippines', 'Sri Lanka', 'Nepal', 'Cambodia', 'Maldives']
  ...
  return (
    <div
      ...
      <div style={{ height: isOpen ? 320 : 180, position: 'relative', overflow: 'hidden', transition: 'height 0.4s ease' }}>
        <picture>
          <source srcSet={`/images/asia/${dest.slug}-mobile.webp`} media="(max-width: 480px)" type="image/webp" />
          <source srcSet={`/images/asia/${dest.slug}-tablet.webp`} media="(max-width: 1024px)" type="image/webp" />
          <source srcSet={`/images/asia/${dest.slug}-desktop.webp`} type="image/webp" />
          <img
            src={`/images/asia/${dest.slug}.jpg`}
            alt={dest.name}
...
export default function AsiaPage() {
  ...
  const groupedByCountry = orderedCountries.map(country => ({
    country,
    items: asiaDestinations.filter(d => d.country === country),
              ...
              <div style={{ background: '#111110', border: '1px solid rgba(200,169,110,0.15)', padding: 'clamp(24px,3vw,40px)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 16 }}>
                  {[
                    ...
                    { heading: 'Thailand / Vietnam / Indonesia', body: 'Visa-free or Visa on Arrival for mostWestern passports. Check updated requirements before travel.' },
                    { heading: 'China', body: 'Visa required for most nationalities. Apply 4–6 weeks ahead through embassy or visa agency.' },
                    { heading: 'Singapore', body: 'Visa-free for 30–90 days for most nationalities. Check ICAwebsite for latest entry rules.' },
                  ].map(item => (
