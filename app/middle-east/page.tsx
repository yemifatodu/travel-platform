...
const countryOrder = ['UAE', 'Saudi Arabia', 'Jordan', 'Oman', 'Turkey', 'Qatar', 'Egypt', 'Israel/Palestine', 'Lebanon', 'Bahrain', 'Kuwait']
...
function DestinationCard({ dest, selectedDest, setSelectedDest }: { dest: any; selectedDest: any; setSelectedDest: (d: any) => void }) {
  ...
  return (
    <div
      ...
      style={{
        background: '#111110',
        border: `1px solid ${isOpen ? gold : 'rgba(200,169,110,0.1)'}`,
        cursor: 'pointer',
        overflow: 'hidden',
      ...
      <div style={{ height: isOpen ? 320 : 180, position: 'relative', overflow: 'hidden', transition: 'height 0.4s ease' }}>
        <picture>
          <source srcSet={`/images/middle-east/${dest.slug}-mobile.webp`} media="(max-width: 480px)" type="image/webp" />
          <source srcSet={`/images/middle-east/${dest.slug}-tablet.webp`} media="(max-width: 1024px)" type="image/webp" />
          <source srcSet={`/images/middle-east/${dest.slug}-desktop.webp`} type="image/webp" />
          <img
...
export default function MiddleEastPage() {
  ...
  const uniqueCountries = [...new Set(middleEastDestinations.map((d: any) => d.country))]
  const unescoCount = middleEastDestinations.filter((d: any) => d.unesco === true).length
  ...
  const groupedByCountry = orderedCountries.map(country => ({
    country,
    items: middleEastDestinations.filter((d: any) => d.country === country),
