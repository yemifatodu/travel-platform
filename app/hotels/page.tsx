...
// ─────────────────────────────────────────────────────────────
// SVG ICON COMPONENTS (all use gold stroke, 1.5px width)
// ─────────────────────────────────────────────────────────────
...
const StarFilledIcon = ({ className = '' }: { className?: string }) => (
  <svg className={className} width="14" height="14" viewBox="0 0 24 24" fill={gold} stroke={gold} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    <path d="M12 2 9.5 8.5 3 9.5l5 4.5-1.5 6.5L12 17l5.5 3.5-1.5-6.5 5-4.5-6.5-1z"/>
  </svg>
)

const CoinIcon = ({ className = '' }: { className?: string }) => (
  <svg className={className} width="32" height="32" viewBox="0 0 24 24" fill="none" stroke={gold} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
const GiftIcon = ({ className = '' }: { className?: string }) => (
  <svg className={className} width="32" height="32" viewBox="0 0 24 24" fill="none" stroke={gold} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    <rect x="3" y="8" width="18" height="13" rx="2"/><path d="M12 8v13M3 12h18M7 8V5a3 3 0 0 1 6 0v3M11 5a3 3 0 0 1 6 0v3"/>
  </svg>
)
...
// Country Flag Badge (simplified gold badge with country code)
...
// ─────────────────────────────────────────────────────────────
// DATA ARRAYS (emojis replaced with SVG components)
// ─────────────────────────────────────────────────────────────
...
const popularDestinations = [
  { city: 'Dubai', country: 'UAE', code: 'AE', from: '$120/night' },
  { city: 'Bali', country: 'Indonesia', code: 'ID', from: '$45/night' },
  { city: 'London', country: 'UK', code: 'GB', from: '$180/night' },
  { city: 'Paris', country: 'France', code: 'FR', from: '$160/night' },
  { city: 'Nairobi', country: 'Kenya', code: 'KE', from: '$80/night' },
  { city: 'Tokyo', country: 'Japan', code: 'JP', from: '$140/night' },
  { city: 'New York', country: 'USA', code: 'US', from: '$200/night' },
  { city: 'Santorini', country: 'Greece', code: 'GR', from: '$220/night' },
  { city: 'Cape Town', country: 'South Africa', code: 'ZA', from: '$90/night' },
  { city: 'Bangkok', country: 'Thailand', code: 'TH', from: '$40/night' },
  { city: 'Maldives', country: 'Maldives', code: 'MV', from: '$300/night' },
  { city: 'Barcelona', country: 'Spain', code: 'ES', from: '$130/night' },
]
...
export default function HotelsPage() {
  useEffect(() => {
    // Check if IMPT script is already loaded
    if (!document.querySelector('script[src*="swarm.impt.io/widget.js"]')) {
      const script = document.createElement('script');
      script.src = 'https://swarm.impt.io/widget.js';
      script.async = true;
      script.setAttribute('data-key', 'p_sqcs0mfl98'); // Your IMPT partner key
      document.head.appendChild(script);
    }

    // Add custom styling to match Huuboi brand
    const style = document.createElement('style');
    style.textContent = `
      /* Target IMPT widget shadow DOM elements */
      .impt-widget-container {
        font-family: 'DM Sans', sans-serif !important;
      }
      
      /* Style the widget's root container */
      #impt-swarm {
        width: 100%;
      }
      ...
      #impt-swarm select,
      #impt-swarm input {
        background: rgba(245, 239, 228, 0.04) !important;
        border: 1px solid rgba(200, 169, 110, 0.2) !important;
        color: #F5EFE4 !important;
        font-family: 'DM Sans', sans-serif !important;
        border-radius: 0 !important;
      ...
      #impt-swarm select:focus,
      #impt-swarm input:focus {
      ...
      #impt-swarm button:hover {
        background: #D8B97E !important;
        transform: translateY(-2px) !important;
        box-shadow: 0 8px 24px -8px rgba(200, 169, 110, 0.35) !important;
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080807', paddingTop: 90 }}>
      ...
      {/* Hero - Integrated with IMPT Widget */}
      <div id="hotel-search-section" style={{ background: 'linear-gradient(160deg,#080a10,#0a080c,#080807)', borderBottom: '1px solid rgba(200,169,110,0.12)', padding: 'clamp(60px,10vw,120px) clamp(20px,5vw,60px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          {/* IMPT Hotel Search Widget Container */}
          <div style={{ background: '#111110', border: '1px solid rgba(200,169,110,0.15)', padding: 'clamp(20px,3vw,32px)', maxWidth: 900, position: 'relative' }}>
            <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '0.6rem', letterSpacing: '0.18em', color: gold, marginBottom: 16 }}>
              SEARCH HOTELS — Powered by IMPT Swarm
            </div>
            
            {/* IMPT Widget mounts here */}
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(48px,7vw,80px) clamp(20px,5vw,60px)' }}>
                ...
                <div style={{ background: hotel.gradient, height: 120, position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', bottom: 12, left: 14 }}>
                    <div style={{ display: 'flex', gap: 2 }}>
                      {[...Array(Math.min(hotel.stars, 5))].map((_, i) => <StarFilledIcon key={i} className="w-3 h-3" />)}
                    </div>
                  </div>
                </div>
        ...
        {/* Popular destinations - Flag badges */}
        <div style={{ marginBottom: 'clamp(48px,7vw,80px)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 2 }}>
            {popularDestinations.map(dest => (
              <div key={dest.city}
                ...
                onMouseEnter={e => (e.currentTarget.style.borderColor = 'rgba(200,169,110,0.35)')}
                onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(200,169,110,0.1)')}>
                <div style={{ marginBottom: 10 }}><FlagBadge code={dest.code} /></div>
        ...
        {/* IMPT Benefits Banner - SVG Icons */}
        <div style={{ background: 'rgba(200,169,110,0.06)', border: '1px solid rgba(200,169,110,0.2)', padding: '28px 32px', marginBottom: 32, textAlign: 'center' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 40 }}>
            <div>
              <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'center' }}><CoinIcon /></div>
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '0.75rem', letterSpacing: '0.12em', color: gold }}>5% COMMISSION</div>
              <div style={{ color: muted, fontSize: '0.7rem' }}>You earn on every booking</div>
            </div>
            <div>
              <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'center' }}><LeafIcon /></div>
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '0.75rem', letterSpacing: '0.12em', color: gold }}>1 TONNE CO₂ OFFSET</div>
              <div style={{ color: muted, fontSize: '0.7rem' }}>Paid by IMPT per stay</div>
            </div>
            <div>
              <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'center' }}><GiftIcon /></div>
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '0.75rem', letterSpacing: '0.12em', color: gold }}>€5 FREE CREDIT</div>
              <div style={{ color: muted, fontSize: '0.7rem' }}>For your guest at signup</div>
            </div>
            <div>
              <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'center' }}><ClockIcon /></div>
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '0.75rem', letterSpacing: '0.12em', color: gold }}>90-DAY COOKIE</div>
              <div style={{ color: muted, fontSize: '0.7rem' }}>Long attribution window</div>
            </div>
          </div>
        </div>
