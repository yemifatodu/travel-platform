...
export default function EsimPage() {
  ...
  const fetchPlans = async () => {
    ...
    try {
      while (hasMore && retryCount < maxRetries) {
        console.log(`Fetching plans with offset: ${offset}`);
      ...
      console.log(`✅ Loaded ${allPlans.length} plans from ${uniqueCountries.length} countries`);
    } catch (error) {
  ...
  const filteredCountries = searchQuery
    ...
    : Object.keys(plansByCountry).sort();
              ...
              <div key={country} style={{ background: "#0a0a08", border: `1px solid ${dim}`, borderRadius: 12, overflow: "hidden" }}>
                ...
                <div style={{ 
                  ...
                  <h2 style={{ 
                    ...
                    {country} ({plansByCountry[country].length} plans)
                  </h2>
                </div>
                ...
                <div style={{ 
                  ...
                  {plansByCountry[country].slice(0, 20).map((plan) => (
                    ...
                    onMouseLeave={(e) => {
                      ...
                      <div style={{ fontSize: "1.75rem", fontWeight: "bold", color: gold, marginBottom: 8 }}>
                        ${plan.retail_price} <span style={{ fontSize: "0.85rem", color: muted }}>{plan.currency}</span>
                      </div>
                      <div style={{ color: cream, fontSize: "0.9rem", marginBottom: 4 }}>
                        {plan.data_amount}
                      </div>
